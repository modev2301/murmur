...
use murmur_sysinfo::{get_system_info, get_wifi_info, detect_vpn, detect_proxy, NetworkInfo};
...
async fn main() {
    ...
    println!("Model:       {}", sys.machine_model.unwrap_or_else(|| "unknown".into()));
    ...
    println!("Public IP:   {}", net.public_ip.unwrap_or_else(|| "unknown".into()));
    println!("Local IP:    {}", net.local_ip.unwrap_or_else(|| "unknown".into()));
    println!("Gateway:     {}", net.gateway.unwrap_or_else(|| "unknown".into()));
    println!("Interface:   {}", net.interface.unwrap_or_else(|| "unknown".into()));
    println!();

    println!("=== WiFi ===");
    if let Some(wifi) = get_wifi_info() {
        println!("SSID:        {}", wifi.ssid);
        println!("BSSID:       {}", wifi.bssid.unwrap_or_else(|| "unknown".into()));
    ...
    if let Some(vpn) = detect_vpn() {
        ...
        println!("Provider:    {}", vpn.provider.unwrap_or_else(|| "unknown".into()));
    } else {
        println!("Detected:    no");
    }
    println!();
    ...
    if let Some(proxy) = detect_proxy() {
        println!("Detected:    yes");
        println!("HTTP:        {}", proxy.http_proxy.unwrap_or_else(|| "none".into()));
        println!("HTTPS:       {}", proxy.https_proxy.unwrap_or_else(|| "none".into()));
    } else {
        println!("Detected:    no");
    }
}
